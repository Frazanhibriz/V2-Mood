...
<style>
    ...
    * {font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif;}
    ...
    .stRadio > div > label > div {color: #4b5563 !important;}
    ...
    .stats-container {background: white; border-radius: 12px; padding: 1.25rem; text-align: center; border: 2px solid #e5e7eb;}
    .stats-label {font-size: 0.8rem; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.4rem;}
    .stats-value {font-size: 1.5rem; font-weight: 700; color: #4f46e5;}
    .stats-subtext {font-size: 0.85rem; color: #9ca3af; margin-top: 0.25rem;}
    ...
    .info-box p {color: #1e40af; font-size: 0.9rem; margin: 0; line-height: 1.4;}
    ...
    .badge {display: inline-block; background: #ede9fe; color: #5b21b6; padding: 0.5rem 1rem; border-radius: 9999px; font-size: 0.9rem; font-weight: 500;}
...
def predict_from_image(image_pil):
...
if st.session_state.step == 1:
    ...
    chosen = st.radio("Select your current mood:", options, index=default_idx, key="mood_rad")
    ...
    with col1:
        ...
        f = st.radio("flav", ["Sweet","Fresh","Creamy","Strong"], label_visibility="collapsed")
        st.session_state.flavor_pref = f.lower()

    with col2:
        st.markdown("**Energy level:**")
        e = st.radio("eng", ["Tired","Normal","Energetic"], index=1, label_visibility="collapsed")
    ...
    if st.button("Start Over"):
        for k in ['detected_cat','detected_conf','final_cat','flavor_pref','energy_level']:
